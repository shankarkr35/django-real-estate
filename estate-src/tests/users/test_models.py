...
def test_user_str(base_user):
    """Test The custom user model string representations"""
...
def test_create_user_with_no_username(user_factory):
    ...
    assert str(err.value) == "Users Must Submit Username"
...
def test_create_user_with_no_firstname(user_factory):
    ...
    assert str(err.value) == "Users Must Submit First Name"
...
def test_create_user_with_no_lastname(user_factory):
    ...
    assert str(err.value) == "Users Must Submit Last Name"

def test_create_superuser_with_no_email(user_factory):
    """Test craeting superuser without an email address raises an error"""
    with  pytest.raises(ValueError) as err:
...
def test_create_superuser_with_no_password(user_factory):
    """Test craeting superuser without apassword raises an error"""
    with  pytest.raises(ValueError) as err:
...
def test_user_email_incorrect(user_factory):
    """Test that an Error is raised when a non valid email address is provided"""
    with pytest.raises(ValueError) as err:
